...
- Admin-side, outside the repo: an age key for the admin and a pre-generated host SSH key, both kept out of git; and a KVM-capable `x86_64-linux` builder, which the VM test requires and which is provisioned before this change starts. No accounts needed (`ACCT`: none).
