...
ACCEPTED_MODE = re.compile(r"[0-9a-fA-F]{4}:[0-9a-fA-F]{4}")
...
class UsageError(Exception):
    """The command line this report was registered with is malformed."""


class _Parser(argparse.ArgumentParser):
    # argparse exits 2 on a malformed command line, which the aggregator
    # would read as this report finding its capability failed. A report
    # registered with a bad command line never ran at all, so the error is
    # raised instead and `main` turns it into the did-not-run status.
    def error(self, message: str) -> NoReturn:
        raise UsageError(message)


def accepted_mode(token: str) -> Mode:
    """Parse one ``vendor:product`` token as given on the command line."""

    if not ACCEPTED_MODE.fullmatch(token):
        raise argparse.ArgumentTypeError(
            f"not a vendor:product pair of four hex digits each: {token!r}"
        )
    vendor, _, product = token.partition(":")
    return Mode(vendor=vendor.lower(), product=product.lower())
...
def joystick_event_nodes(udev_db: str) -> list[str]:
    """Every input event device node udev's database marks a joystick.

    ``udevadm info --export-db`` prints one block per device, blocks
    separated by a blank line, each property on an ``E:`` line. A joystick's
    ``jsN`` sibling carries the same mark as its event device; only event
    nodes are kept, since those are what the port rule links.
    """

    nodes = []
    for block in udev_db.split("\n\n"):
        properties = [
            line.removeprefix("E: ") for line in block.splitlines() if line.startswith("E: ")
        ]
        devname = next(
            (prop.removeprefix("DEVNAME=") for prop in properties if prop.startswith("DEVNAME=")),
            None,
        )
        if (
            devname is not None
            and devname.startswith("/dev/input/event")
            and "ID_INPUT_JOYSTICK=1" in properties
        ):
            nodes.append(devname)
    return sorted(nodes)
...
def discover_joysticks(*, sys_class_input: Path = Path("/sys/class/input")) -> list[Joystick]:
    ...
    identity the device itself reports, from one read of udev's database."""

    udev_db = subprocess.check_output(["udevadm", "info", "--export-db"], text=True)
    joysticks = []
    for devnode in joystick_event_nodes(udev_db):
        event = devnode.rsplit("/", 1)[-1]
        vendor = _input_id(event, "vendor", sys_class_input=sys_class_input)
        product = _input_id(event, "product", sys_class_input=sys_class_input)
        # Gone between the database read and this one: nothing to classify.
