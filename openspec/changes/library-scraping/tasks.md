...
- [x] 2.5 Declare the kiosk's optional pre-frontend step option, which carries a contributed step's text and defaults to none, so `modules/library` can set it in group 4 before the session loop runs it in group 5; with the option unset the rendered session script is unchanged; proven by an evaluation check that the default leaves the session script byte-identical to the one rendered before the option existed
