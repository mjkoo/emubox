...
- Three packages appear under `pkgs/`, exposed through the existing overlay and as `packages.x86_64-linux` outputs, so `nix build .#packages.x86_64-linux.{es-de,freeimage,duckstation}` succeeds from the Mac through its Linux builder and CI's host build compiles them:
  - `freeimage`: the pre-removal nixpkgs derivation (`3.18.0-unstable-2024-04-18`, revision `7c7c704523`, the parent of the removal) unchanged apart from a provenance header and the three small fixes forward the newer libjpeg, OpenEXR and libtiff in the pinned nixpkgs forced (design D2), including its unbundling diff, its eight CVE patches and its `knownVulnerabilities` list of more than thirty unpatched CVEs. The flake names it in `permittedInsecurePackages` with the risk acknowledged in place: FreeImage only ever decodes images the admin put on the box (scraped art, theme assets), no untrusted input reaches it, and CI builds it whenever its inputs change so a breaking library bump surfaces before the box.
