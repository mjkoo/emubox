...
- Bumping: edit `version` to the new release, set `hash = lib.fakeHash`, rebuild, and record the hash the failed fetch reports; the URL is derived from `version`, so nothing else in the file changes. `nix-update` is not used: `wrapType2` consumes `src` as the `buildFHSEnv` run target and the resulting derivation exposes no `src` attribute for it to read. No `sources.json` or `update.sh` since there are no sub-sources.
...
- [The FHS wrapper's library closure is large] → `buildFHSEnv` pulls a broad default library set (Qt, GL, X11, audio) into DuckStation's closure, and the disko installer VM must hold the whole host closure on its disk. If the install test fails on space, raise the VM's disk size in `tests/`; nothing in the wrapper is trimmed speculatively.
...
None that change the specs, the approach or the tasks. The exact install path of `es_find_rules.xml` in 3.4.1's install tree is confirmed while writing the guard (D2); whichever it is, the guard is a one-line difference.
