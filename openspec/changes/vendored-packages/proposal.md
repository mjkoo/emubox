...
- Three packages appear under `pkgs/`, exposed through the existing overlay and as `packages.x86_64-linux` outputs, so `nix build .#es-de .#freeimage .#duckstation` succeeds on a Linux builder and CI's host build compiles them:
  ...
  - `duckstation`: a wrap of the upstream `DuckStation-x64.AppImage` at release `v0.1-11752`, not a source build. The repository and the Cachix cache are public, so a patched source build pushed to the cache would be public distribution of a modified CC-BY-NC-ND work, which the ND clause forbids; an unmodified upstream binary, attributed and non-commercial, is what the licence permits, and the AppImage is the author's own distribution channel. Bumping it is a version and hash edit.
...
- `pkgs/default.nix` (three `callPackage`s), `pkgs/freeimage/` (new: `package.nix`, `unbundle.diff`, `libtiff-4.4.0.diff`, eight `CVE-*.patch`), `pkgs/es-de/package.nix` (new), `pkgs/duckstation/package.nix` (new), `overlays/default.nix` (unchanged in content), `flake.nix` (`permittedInsecurePackages` next to `allowUnfree`; `packages` restricted to `x86_64-linux` and built from a nixpkgs that carries that config and the overlay, so the standalone package builds honour the same policy the host does and `nix flake check` on macOS is not asked to evaluate Linux-only packages), `modules/kiosk` and `modules/emulators` (one package each), `tests/default.nix` (one subtest group), `README.md`, `.github/workflows/ci.yml` (a comment only: the push step no longer "only uploads").
